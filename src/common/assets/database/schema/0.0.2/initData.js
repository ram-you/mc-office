  ...
  return new Promise(function(resolve, reject) {
    ...
    function countInvoices() {
      knex('invoices').count('* as count').then(c => {
        ...
        if (invoicesCount == 0) { createInvoicesData() } else { resolve(true) }
      })
    }
  ...
  return new Promise(function(resolve, reject) {
    ...
    function countUsers() {
      knex('users').count('* as count').then(c => {
        ...
        if (usersCount == 0) { createUsersData() } else { resolve(true) }
      })
    }
...
function initDatabase() {
  ...
  promises.then(function() {
    ...
    mainWindow.webContents.send("initApplicationData", 'finish');
  });
}
initDatabase()
module.exports = {}