...
let schemaModels = require("./models")
  ...
  function initTable(tableName) {
    return new Promise(function(resolve, reject) {
        knex.schema.hasTable(tableName).then(function(exists) {
          if (!exists) {
            var tableSchemaModel = require("./models/" + schemaModels[tableName])
        ...
        function importDataFromSqlFile(tableName) {
          var dataSqlFile = "./data/" + tableName + ".sql"
          fs.exists(dataSqlFile, function(exists) {
              if (exists) {
                mainWindow.webContents.send("initApplicationData", 'start');
                fs.readFile(dataSqlFile, function readFileCallback(err, sql) {
    ...
    function initDatabase() {
      ...
      var funcs = []
      for (var i = 0; i < databaseModels.lenght; i++) {
        funcs.push(databaseModels[i] + '()')
      }
