 ...
 const actions = {
   ...
   login({ commit }, data) {
     ...
     return new Promise((resolve, reject) => {
       db.Users.findOne({ username: data.username, password: pass_hash }, function(err, doc) {
         if (doc) {
           if (data.remember) {
   ...
   getUser({ commit }, data) { 
     return db.Users.findOne({ username: data.username }, function(err, doc) {
       if (doc) {
  ...
  //   })
   ...
   isConnected({ commit }) {
     ...
     if (userName && userPassword) {
       db.Users.findOne({ username: userName, password: userPassword }, function(err, doc) {
         if (doc) {
           var user = { username: doc.username, connected: true }
           commit("setUser", user);
